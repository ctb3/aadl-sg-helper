 ...
 * (GCP_SA_KEY_SSM_PARAM), never values — so nothing secret lands in Terraform
 * state or lambda:GetFunctionConfiguration output. The server awaits this
 * before listen(); values are fetched once per cold start.
 *
 * Local dev is unchanged: a value already present in the env
 * (GOOGLE_APPLICATION_CREDENTIALS instead of GCP_SA_KEY_JSON) wins, and
