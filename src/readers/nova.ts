...
export const novaReader: Reader = {
  name: "nova",
  async read(image: Buffer, _arm: Arm): Promise<ReaderResult> {
    const res = await converseVision(config.novaModel, image, {
    ...
    return {
      ...
      costUsd: tokenCost(res.inputTokens, res.outputTokens, config.cost.novaInPerMTok, config.cost.novaOutPerMTok),
    };
  },
};
